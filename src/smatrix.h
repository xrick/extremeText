...
// Sparse row matrix, type T needs to contain int at offset 0!
template <typename T>
class SRMatrix {
public:
    SRMatrix();
...
template <typename T>
inline SRMatrix<T> SRMatrix<T>::transpose() {
    SRMatrix<T> tMatrix;
    return tMatrix;
}

template <typename T>
inline void SRMatrix<T>::unitNormRows(){
    for(int i = 0; i < m; ++i)
        unitNorm(r[i], s[i]);
}
